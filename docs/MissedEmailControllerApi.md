...
try {
    val result : MissedEmailDto = apiInstance.getMissedEmail(missedEmailId)
    println(result)
} catch (e: ClientException) {
...
try {
    ...
    println("4xx response calling MissedEmailControllerApi#restoreMissedEmails")
    e.printStackTrace()
} catch (e: ServerException) {
    println("5xx response calling MissedEmailControllerApi#restoreMissedEmails")
    e.printStackTrace()
}
```
...
val before : java.time.OffsetDateTime = 2013-10-20T19:20:30+01:00 // java.time.OffsetDateTime | Filter by created at before the given timestamp
try {
    val result : MissedEmailDto = apiInstance.waitForNthMissedEmail(index, inboxId, timeout, since, before)
    println(result)
} catch (e: ClientException) {
