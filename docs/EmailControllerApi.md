...
# **getEmailHTMLJson**
> EmailHtmlDto getEmailHTMLJson(emailId, decode)
...
val decode : kotlin.Boolean = true // kotlin.Boolean | 
try {
    val result : EmailHtmlDto = apiInstance.getEmailHTMLJson(emailId, decode)
    println(result)
} catch (e: ClientException) {
    println("4xx response calling EmailControllerApi#getEmailHTMLJson")
    e.printStackTrace()
} catch (e: ServerException) {
    println("5xx response calling EmailControllerApi#getEmailHTMLJson")
    e.printStackTrace()
}
```
