...
**virtualInbox** | **kotlin.Boolean** | Virtual inbox can receive email but will not send emails to real recipients. Will save sent email record but never send an actual email. Perfect for testing mail server actions. | 
**domainId** | [**java.util.UUID**](java.util.UUID) | ID of custom domain used by the inbox if any |  [optional]
