...
<a name="getEmails"></a>
# **getEmails**
> kotlin.collections.List&lt;EmailPreview&gt; getEmails(inboxId, aliasForLimitAssessedFirstBeforeAssessingAnyPassedLimit, limit, sortTheResultsByReceivedDateAndDirectionASCOrDESC, retryTimeout, delayTimeout, minCount, unreadOnly, before, since)
...
try {
    val result : kotlin.collections.List<EmailPreview> = apiInstance.getEmails(inboxId, aliasForLimitAssessedFirstBeforeAssessingAnyPassedLimit, limit, sortTheResultsByReceivedDateAndDirectionASCOrDESC, retryTimeout, delayTimeout, minCount, unreadOnly, before, since)
    println(result)
} catch (e: ClientException) {
