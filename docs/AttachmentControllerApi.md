...
try {
    val result : PageAttachmentEntity = apiInstance.getAttachments(fileNameFilter, page, size, sort)
    println(result)
} catch (e: ClientException) {
...
> kotlin.collections.List&lt;kotlin.String&gt; uploadAttachmentBytes(string, filename, byteArray)
...
try {
    val result : kotlin.collections.List<kotlin.String> = apiInstance.uploadAttachmentBytes(string, filename, byteArray)
    println(result)
} catch (e: ClientException) {
