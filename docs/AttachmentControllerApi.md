...
try {
    val result : PageAttachmentEntity = apiInstance.getAttachments(page, size, sort, fileNameFilter, since, before)
    println(result)
} catch (e: ClientException) {
...
> kotlin.collections.List&lt;kotlin.String&gt; uploadAttachmentBytes(inlineObject1, contentType, filename)
...
try {
    val result : kotlin.collections.List<kotlin.String> = apiInstance.uploadAttachmentBytes(inlineObject1, contentType, filename)
    println(result)
} catch (e: ClientException) {
