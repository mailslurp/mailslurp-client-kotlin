...
> kotlin.collections.List&lt;EmailPreview&gt; waitForMatchingEmail(matchOptions, count, delay, inboxId, since, sort, timeout, unreadOnly)
...
val unreadOnly : kotlin.Boolean = true // kotlin.Boolean | Optional filter for unread only
try {
    val result : kotlin.collections.List<EmailPreview> = apiInstance.waitForMatchingEmail(matchOptions, count, delay, inboxId, since, sort, timeout, unreadOnly)
    println(result)
} catch (e: ClientException) {
