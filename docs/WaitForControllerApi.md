...
# **waitForEmailCount**
> kotlin.collections.List&lt;EmailPreview&gt; waitForEmailCount(before, count, delay, inboxId, since, sort, timeout, unreadOnly)
...
> kotlin.collections.List&lt;EmailPreview&gt; waitForMatchingEmails(matchOptions, before, count, delay, inboxId, since, sort, timeout, unreadOnly)
...
val unreadOnly : kotlin.Boolean = true // kotlin.Boolean | Optional filter for unread only
try {
    val result : Email = apiInstance.waitForNthEmail(before, delay, inboxId, index, since, sort, timeout, unreadOnly)
    println(result)
} catch (e: ClientException) {
