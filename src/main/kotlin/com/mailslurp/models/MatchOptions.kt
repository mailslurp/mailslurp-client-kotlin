 ...
 * @param matches 1 or more match options. Options are additive so if one does not match the email is excluded from results
 */

data class MatchOptions (
    /* 1 or more match options. Options are additive so if one does not match the email is excluded from results */
    @Json(name = "matches")
    val matches: kotlin.Array<MatchOption>? = null
)
